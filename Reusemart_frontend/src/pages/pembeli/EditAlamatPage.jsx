...
import { EditAlamat, GetAlamatById } from "../../api/apiAlamat";
...
const EditAlamatPage = () => {
    ...
    useEffect(() => {
        const fetchAlamat = async () => {
          try {
            const data = await GetAlamatById(id);
            const cleanedData = { ...data.data };
            setAlamatData(cleanedData);
            setOriginalData(cleanedData);
            setIsCheckDis(cleanedData.is_default);
          } catch (error) {
    ...
    useEffect(() => {
        setIsChanged(JSON.stringify(alamatData) !== JSON.stringify(originalData));
    }, [alamatData, originalData]);
